...
export const CancelOrderRequestModal: React.FC<CancelOrderRequestModalProps> = ({ open, order, onClose, onSubmit }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-4 border-b border-border-subtle flex justify-between items-center bg-stone-50 shrink-0">
                    <div>
                        <h3 className="text-xl font-bold font-['Lora',serif] text-stone-900">Hủy đơn hàng</h3>
                        <p className="text-sm text-stone-500 mt-1">Bạn sắp hủy đơn hàng {order.id}</p>
                    </div>
                    <button 
                        onClick={onClose} 
                ...
                <div className="p-6 overflow-y-auto flex-1">
                    <form id="cancel-order-form" onSubmit={handleSubmit} className="flex flex-col gap-6">
                        {/* Products */}
                        <div>
                        ...
                        {/* Reason */}
                        <div>
                            <label htmlFor="cancel-note" className="block font-semibold text-stone-800 mb-2 text-sm">
                                Lý do hủy đơn hàng <span className="text-market-error">*</span>
                            </label>
                            <textarea
                                id="cancel-note"
                                ref={textareaRef}
                                ...
                                className="w-full min-h-30 p-3 border-2 border-border-medium rounded-xl focus:border-market-error focus:ring-4 focus:ring-market-error/20 outline-none transition-all resize-y text-sm bg-stone-50 focus:bg-white disabled:opacity-60"
                            />
                        </div>
                    </form>
                </div>
                ...
                <div className="px-6 py-4 border-t border-border-subtle bg-stone-50 flex items-center justify-end gap-3 shrink-0">
                    ...
                    <button
                        ...
                        className="px-6 py-2 font-medium text-white bg-market-error hover:bg-red-700 rounded-xl transition-colors disabled:opacity-60 min-w-45 flex items-center justify-center gap-2 shadow-sm"
                    >
                        {isSubmitting && (
