...
import { DefaultIcon, type ShippingDetailFormProps } from './shippingForm.type';
import { LocationSelector, useShippingDetailController } from './shippingForm.controller';

L.Marker.prototype.options.icon = DefaultIcon;

export const ShippingDetailForm: React.FC<ShippingDetailFormProps> = ({ address }) => {
    const { position, setPosition } = useShippingDetailController(address.latitude, address.longitude);
        ...
        <section className="w-full">
            ...
            <div className="bg-surface-card card-border rounded-lg p-6">
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    {/* Phần Leaflet Map (Bên phải) */}
                    <div className="w-full lg:w-[320px] xl:w-[380px] shrink-0 flex flex-col gap-2">
                        <div className="h-[280px] lg:h-full min-h-[280px] rounded-lg border border-border-medium overflow-hidden z-0">
                            <MapContainer
                                center={position}
                                zoom={15}
                        ...
                        <div className="text-center">
                            <p className="font-caption text-text-muted">
                                Click trên bản đồ để chọn lại vị trí
                            </p>
                        </div>
                    </div>
